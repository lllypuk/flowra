...
// MockMessageRepository - мок репозитория сообщений for tests
type MockMessageRepository struct {
	Messages map[uuid.UUID]*domainMessage.Message
	SaveErr  error
}

// NewMockMessageRepository creates New мок репозитория
...
// FindByChatID finds messages по ID chat
...
// FindThread finds тред сообщений
...
// CountByChatID подсчитывает messages in чате
...
// CountThreadReplies подсчитывает responses in треде
...
// GetReactionUsers returns users, поставивших реакцию
...
// contains checks, contains ли строка подстроку (case-insensitive)
...
// MockChatRepository - мок репозитория chats for tests
type MockChatRepository struct {
	Chats map[string]*chatapp.ReadModel
}

// NewMockChatRepository creates New мок репозитория chats
...
// AddChat добавляет chat с участниками
...
// MockEventBus - мок шины events for tests
type MockEventBus struct {
	Published []event.DomainEvent
}

// NewMockEventBus creates New мок шины events
