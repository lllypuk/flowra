...
func TestDockerComposeProd_KeycloakRealmImportAndAppDependency(t *testing.T) {
	...
	require.Contains(t, joinedHealthcheck, "HEAD /health/ready HTTP/1.0")
	require.Contains(t, joinedHealthcheck, "HTTP/1.0 200")
...
func TestDockerComposeProd_MongoReplicaSetReadinessAndDependencies(t *testing.T) {
	...
	require.Contains(t, joinedHealthcheck, "db.adminCommand('ping').ok")
