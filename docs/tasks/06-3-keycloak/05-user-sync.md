...
### Testing
- [ ] Unit tests для worker
- [ ] Integration test с Keycloak

### Integration
- [ ] Worker запускается в cmd/worker
- [ ] Graceful shutdown
...
- [ ] Новые пользователи из Keycloak создаются в MongoDB
- [ ] Изменения профиля синхронизируются
- [ ] Удалённые/disabled пользователи деактивируются
- [ ] Sync запускается каждые 15 минут
- [ ] Логи показывают прогресс sync
- [ ] Sync не блокирует приложение
